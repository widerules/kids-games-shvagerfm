...
local data = require ("data.FVFallData")
...
local popupText, popupBg, nextBtn, homeBtn, roloadBtn --pop-up variable
...
local function showPopUp (message)
    ...
    for i = 1,3 do
    	...
    	rateStars[i].width = 1.2*_INFOITEMSIZE
    	rateStars[i].height = 1.2*_INFOITEMSIZE
    	rateStars[i].y = popupText.y + _FONTSIZE + _INFOITEMSIZE
    	rateStars[i].x = popupBg.x + 1.5*1.2*(i-2)*_INFOITEMSIZE    		
    end
...
function scene:willEnterScene(event)
	...
	generated = 0
	loopNumber = 0
	_ITEMSIZE = constants.H / data.difficults[level].size
end
...
function scene:enterScene (event)
	...
	transition.to(starTypeImage, {time=500, xScale = 1.2, yScale=1.2, x = informationBackground.x - starTypeImage.width, y = informationBackground.y+starTypeImage.height/10, onComplete= listener})
	end

	informationBackground = display.newImage("images/informationbg.png", constants.CENTERX, _INFOITEMSIZE/2)
	informationBackground.width = constants.W/3
	informationBackground.height = 1.5*_INFOITEMSIZE
	informationBackground.x = constants.W - informationBackground.width/2
	informationGroup:insert(informationBackground)

	starTypeImage = display.newImage( data.itemPath[1]..data.colors[colors[index]]..data.format, _INFOITEMSIZE, _INFOITEMSIZE )
	starTypeImage.width = _INFOITEMSIZE
	starTypeImage.height = _INFOITEMSIZE
