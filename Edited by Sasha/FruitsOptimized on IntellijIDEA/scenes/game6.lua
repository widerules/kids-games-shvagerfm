...
local function sayName(event)
    local __soundPath
    if (index[1] < 10) then
        __soundPath = "sounds/"..data.vegetables[index[1]]..".mp3"
    else
        __soundPath = "sounds/"..data.fruits[(index[1]%9)+1]..".mp3"
    end
    soundName = audio.loadSound( __soundPath )
    audio.play( soundName )
end
...
local function createBut(group)  --создаём точки
    ...
    for i = 1, 3 do
        if (i == t) then
            local __path
            if (tmpIndex[i] < 10) then
                __path = data.pathToItems..data.vegetables[tmpIndex[i]]..data.format
            else
                __path = data.pathToItems..data.fruits[(tmpIndex[i]%9)+1]..data.format
            end
            ...
            downImg[#downImg+1] = display.newImage(__path)
...
function scene:enterScene(event)
    ...
    index[1] = math.random (1, 18)
    index[2] = index[1]
    index[3] = index[1]

    while (index[2] == index[1]) do
        index[2] = math.random(1, 18)
    end

    while (index[3] == index[1] or index[3] == index[2]) do
        index[3] = math.random(1, 18)
    end

    for i = 1, #lvlInfo do

        local __path
        if (index[lvlInfo[i]] < 10) then
            __path = data.pathToItems..data.vegetables[index[lvlInfo[i]]]..data.format
        else
            __path = data.pathToItems..data.fruits[(index[lvlInfo[i]]%9)+1]..data.format
        end

        images[#images+1] = display.newImage(__path)
