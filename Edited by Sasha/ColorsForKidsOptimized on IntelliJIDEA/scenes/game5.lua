...
local function backHome()
    storyboard.gotoScene( "scenes.menu", "slideRight", 800 )
...
local function sayName(event)
    local __soundPath = "sounds/"..data.colors[index[1]]..".mp3"
    print (__soundPath)
...
local function createBut(group)  --создаём точки
    ...
    for i = 1, 3 do
        if (i == t) then
            local __path = data.butterfliesPath..data.colors[tmpIndex[i]]..data.format
...
function scene:enterScene(event)
    ...
    index[1] = math.random (1, 7)
    index[2] = index[1]
    index[3] = index[1]

    while (index[2] == index[1]) do
        index[2] = math.random(1, 7)
    end

    while (index[3] == index[1] or index[3] == index[2]) do
        index[3] = math.random(1, 7)
    end

    for i = 1, #lvlInfo do

        local __path = data.butterfliesPath..data.colors[index[lvlInfo[i]]]..data.format
